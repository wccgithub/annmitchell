...
{
  double delta;
  int i, n, max_target, max_output;
  double max_target_value, max_output_value;

  n = net->output_n;
  max_target_value = 0.0;
  max_output_value = 0.0;
  *err = 0.0;
  for(i = 1; i <= n; i++) {
    delta = net->target[i] - net->output_units[i];
    *err += (0.5 * delta * delta);
    if(max_target_value < net->target[i]) {
      max_target_value = net->target[i];
      max_target = i;
    }
    if(max_output_value < net->output_units[i]) {
      max_output_value = net->output_units[i];
      max_output = i;
    }
  }
  //printf("%d    %d", max_target, max_output);
